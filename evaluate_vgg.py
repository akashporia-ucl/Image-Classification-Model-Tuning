...
def load_collated_model_state(hdfs_model_path="/data/model_collated/vgg16_final.pt"):
    """
    Downloads the collated VGG16 model from HDFS to a temporary file,
    ...
    try:
        ...
        print("Error loading collated VGG16 model from HDFS:", e)
        state_dict = None
    finally:
...
def process_partition(iterator, mapping_bc, model_state_dict_bc):
    ...
    For each image in the partition:
      - Loads and preprocesses the image (VGG16 expects 224x224 input).
      - Runs inference using the VGG16 model loaded with the broadcast state dict.
    ...
    # VGG16 transformation: Resize to 256, CenterCrop to 224.
...
def main():
    spark = SparkSession.builder.appName("EvaluateVGG16Model").getOrCreate()
    ...
    # Step 2: Load the collated VGG16 model state dictionary.
    model_state_dict = load_collated_model_state("/data/model_collated/vgg16_final.pt")
    if model_state_dict is None:
        print("Failed to load the collated VGG16 model. Exiting.")
        sys.exit(1)
    ...
    output_path = "/data/model_collated/test_results_vgg"
